...
#         # print(self.grid)
#
#     def forward(self, input1, **kwargs):
#         self.input1 = input1
...
#
#         for i in range(input1.size(0)):
#             output = torch.bmm(self.batchgrid.view(-1, self.height * self.width, 3),
#                                torch.transpose(input1, 1, 2)).view(-1, self.height, self.width, 2)
#
#         return output
#
...
#         super().__init__()
#         self.height, self.width = height, width
#         self.f = AffineGridGenFunction(self.height, self.width)  # type: Function
#
...
#         self.grid = np.zeros([self.height, self.width, 3], dtype=np.float32)
#         self.grid[:, :, 0] = np.expand_dims(
#             np.repeat(np.expand_dims(np.linspace(-1, 1, self.height), 0), repeats=self.width, axis=0).T, 0)
#         self.grid[:, :, 1] = np.expand_dims(
...
#         for i in range(input1.size(0)):
#             output = torch.bmm(self.batchgrid.view(-1, self.height * self.width, 3),
#                                torch.transpose(input1, 1, 2)).view(-1, self.height, self.width, 3)
#
#         return output
#
#     def backward(self, grad_output):
...
#             grad_input1 = grad_input1.cuda()
#         grad_input1 = torch.baddbmm(grad_input1,
#                                     torch.transpose(grad_output.view(-1, self.height * self.width, 3), 1, 2),
#                                     self.batchgrid.view(-1, self.height * self.width, 3))
#
#         return grad_input1
#
