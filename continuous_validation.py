...
def alignment_step(config, dataset_lookup=None, model_mode='best_validation', percent_range=None):
    ...
    for x in dataloader:
        ...
        if dataset_lookup == "validation_set":
            # We only care about the hyper-parameter postprocessing seach for the validation set
...
def main():
    ...
    if mode in ['all', 'validation', 'init']:
        ...
        error, i_error, mi_error, _, _, _ = alignment_step(config, dataset_lookup='validation_set',
