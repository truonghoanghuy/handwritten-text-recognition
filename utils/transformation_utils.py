...
def compute_renorm_matrix(img):
    ...
    inv_c = torch.from_numpy(inv_c).type_as(img)
    inv_b = torch.from_numpy(inv_b).type_as(img)

    return inv_b.mm(inv_c)


def compute_next_state(delta, state):
    out = torch.zeros(*state.data.shape).type_as(state)
    for i in range(0, 3):
...
def compute_points(state):
    out = torch.zeros(state.data.shape[0], 2, 2).type_as(state)
...
def compute_perspective(pts, target=None):
    global BASIS
    if target is None:
        target = torch.from_numpy(DEFAULT_TARGET).type_as(pts)
    if BASIS is None:
...
def pt_ori_sca_2_pts(state):
    ...
    out = torch.ones(state.data.shape[0], 3, 2).type_as(state)
...
def get_init_matrix(input):
    output = torch.zeros((input.size(0), 3, 3)).type_as(input)
    output[:, 0, 0] = 1
    output[:, 1, 1] = 1
    output[:, 2, 2] = 1
...
class ScaleRotateMatrixGenerator(Module):
    ...
    def forward(self, input: torch.Tensor):
        output = torch.zeros((input.size(0), 3, 2)).type_as(input)
        output[:, 0, 0] = 1
        output[:, 1, 1] = 1
