...
class LMStats(object):
    def __init__(self, default_weight=None):
        self.accumulated_probabilities = None
        self.count = 0
    ...
    def add_stats(self, data):
        ...
        if self.accumulated_probabilities is None:
            self.accumulated_probabilities = softmax_data.sum(axis=0)
        else:
            self.accumulated_probabilities += softmax_data.sum(axis=0)

    def get_state(self):
        char_log_prob = np.log(self.accumulated_probabilities / self.count)
