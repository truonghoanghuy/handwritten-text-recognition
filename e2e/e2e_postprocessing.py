...
def filter_on_pick_no_copy(out, pick):
    output = {'sol': out['sol'][pick], 'lf': [li[pick] for li in out['lf']], 'hw': out['hw'][pick]}
    if 'idx' in out:
...
def postprocess(out, **kwargs):
    ...
    if lf_nms_params is not None:
        ...
        lf_setup = np.concatenate([li[None, ...] for li in out['lf']])
...
def read_order(out):
    ...
    first_pt = first_pt.tolist()  # type: np.ndarray
