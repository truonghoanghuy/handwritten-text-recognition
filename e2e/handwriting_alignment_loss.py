...
def accumulate_scores(out, out_positions, xy_positions, gt_state, idx_to_char):
    ...
    # preds_size = torch.tensor([preds.size(0)] * batch_size, dtype=torch.int)
...
def update_alignment(out, gt_lines, alignments, idx_to_char, idx_mapping, sol_positions: torch.Tensor):
    # preds = out.cpu()
    # batch_size = preds.size(1)
    # preds_size = torch.tensor([preds.size(0)] * batch_size, dtype=torch.int)
...
def alignment(predictions, hw_scores, alpha_alignment=0.1):
    confidences = predictions[:, :, 4]

    log_confidences = torch.log(confidences + 1e-10)
    log_one_minus_confidences = torch.log(1.0 - confidences + 1e-10)

    expanded_log_confidences = log_confidences[:, :, None].expand(confidences.size(0), confidences.size(1),
                                                                  hw_scores.size(2))
    expanded_log_one_minus_confidences = log_one_minus_confidences[:, :, None].expand(confidences.size(0),
                                                                                      confidences.size(1),
                                                                                      hw_scores.size(2))
...
def loss(preds, non_hw_sol, hw_sol, gt_lines, char_to_idx, criterion):
    ...
    # print " - - - - Losses - - - - "
    # print ctc_loss.item()
    # print selected_confidence.item(), log_confidences.size()
    # print not_selected_confidence.item(), log_one_minus_confidences.size()
    # print ""
